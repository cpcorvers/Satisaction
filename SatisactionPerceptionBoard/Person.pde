...
// ]
  ...
  Person(float pawn_x, float pawn_y, float pawn_polarity, float pawn_intensity, float pawnCenterX, float pawnCenterY, float identity, float direction, int timestamp){
    pawn_x = pawn_x;
    pawn_y = pawn_y;
 ...
 public void showPawn(float pawnCenterX, float pawnCenterY, float identity, float direction) {
    pushMatrix();
    translate(pawn_x + imagePawnWidth/2, pawn_y - imagePawnHeight/2);
    rotate(direction + (PI / 2));
    image(pawnVisual, 0, 0, imagePawnWidth*2.5, imagePawnHeight*2.5);
    textSize(20);
    fill(0, 102, 153);
    text(identity, 20, -50);
    popMatrix();
...
void showPawnTesting(float pawnCenterX, float pawnCenterY, float identity, float direction, float intensity) {
    pushMatrix();
    ...
    text(identity, 10, 70);
    textSize(40);
    //text("intensity: " + intensity, 0,-150);
    popMatrix();
}
  ...
  void becomeJSONObjectPawn() {
    ...
    pawn.setInt("timestamp", timestamp);
